...
export default function ServiceDetail() {
  ...
  const displayImages = derivedVariant?.images?.length > 0 ? derivedVariant.images : (service?.images || [])
  ...
  if (!service) return (
    <section className="pt-32 pb-24 px-4 text-center">
      ...
      <Link to="/servicios" style={{ color: "var(--color-primary)" }} className="hover:underline">Volver a servicios</Link>
    </section>
  )
    ...
    <>
      <section className="relative overflow-hidden px-4 sm:px-6 lg:px-8" style={{ backgroundColor: "var(--color-background)", paddingTop: "5rem", paddingBottom: "3rem" }}>
        ...
        <div className="relative max-w-4xl mx-auto">
          <button onClick={() => navigate(-1)} className="inline-flex items-center gap-2 mb-8 transition-colors" style={{ color: "var(--color-text-muted)" }} onMouseEnter={e => e.currentTarget.style.color = "var(--color-primary)"} onMouseLeave={e => e.currentTarget.style.color = "var(--color-text-muted)"}>
